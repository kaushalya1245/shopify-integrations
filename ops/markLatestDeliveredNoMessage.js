...
const processedDeliveriesPath = path.resolve(__dirname, "..", "processed-deliveries.json");
...
async function main() {
  ...
  // 1) Pre-mark as notified so webhook won't send message.
  const notified = loadSet(processedDeliveriesPath);
  const idempotencyKey = String(fulfillmentId);
  const wasAlreadyMarked = notified.has(idempotencyKey);
  if (!wasAlreadyMarked) {
    notified.add(idempotencyKey);
    saveSet(processedDeliveriesPath, notified);
  }

  // 2) Mark delivered in Shopify (creates fulfillment event).
    ...
    JSON.stringify(
      {
        ...
        fulfillmentId,
        preMarkedNoMessage: true,
        alreadyPreMarked: wasAlreadyMarked,
        processedDeliveriesPath,
